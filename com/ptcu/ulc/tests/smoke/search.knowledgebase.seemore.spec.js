/**
 * Created by rdewangan on 07-08-2017.
 */
...
var KnowledgeBasePage = require('../../pages/knowledgeBase.new.seemore.page');
...
var knowledgeBaseArticle = new KnowledgeBasePage()

var searchKeyword = "rounds";


describe('Search knowledge base seemore page validation', function () {
    ...
    it('search icon and search keyword should be present', function () {
        searchResultsPage.knowledgebaseSeeMoreLink.click();
    ...
    it('should validate left facet content type & KnowledgeBase Article Header  ', function () {
        expect(tutorialSeeMoreNew.facets().getCheckedContentType().getText()).toBe("Knowledge Base Articles");
    });
    ...
    it('click see more and results should get appended', function () {
        var countBeforeResults = knowledgeBaseArticle.articles().getCount();
        knowledgeBaseArticle.btnSeeMore().click();
        browser.waitForAngular();
        expect(knowledgeBaseArticle.articles().getCount()).toBeGreaterThan(countBeforeResults);
    });
